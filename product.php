    ...
    <div class="webshop__content">
        <div class="catalogue">
            <?php foreach ($catalogue->getAllProducts() as $product): ?>
                <?php if($product->getCode() == $_GET["productID"]){ ?>
                    <div class="product">
                    <h3><?php echo $product->getTitle() ?></h3>
                    <p><?php echo $product->getDescription() ?></p>
                    <img src="<?php echo $product->getImageURL() ?>" alt="Foto van <?php $product->getTitle() ?>">
                    <p>€<?php echo $product->getPrice() ?></p>
                    <a href="cart.php?action=add_product&code=<?php echo $product->getCode() ?>" class="cart-button">Toevoegen aan winkelmandje</a>
                    <a href="index.php" class="cart-button">Terug naar home pagina.</a>
                </div>
                <?php } ?>
            <?php endforeach; ?>
        </div>
